...
public final class DrawNumberControllerImpl implements DrawNumberController {
    ...
    public void newAttempt(final int n) {
        var attempt=model.attempt(n);  //in this way with multiple views we only propagate the output doing the attempt only once
        for(DrawNumberView v :view){
            Objects.requireNonNull(v, "There is no view attached!").result(attempt);
        }
    }
